...
export interface OrderRegistryPassenger {
  ...
  // Кеп (28.08): реальний код знижки (psgr_dscnt), який МИ САМІ відправили бекенду для
  // цього пасажира в момент бронювання — може бути ПІДМІНЕНИЙ (напр. знижка рейсу замінила
  // категорійну). Порівнюється з ЖИВИМ dsc з бекенду при кожному перегляді: якщо
  // збігається — показуємо наш зафіксований напис "Застосовано знижку рейсу"; якщо
  // відрізняється (менеджер змінив вручну) — довіряємо бекенду повністю, без напису.
  sentDiscountId?: string
  usedTripDiscount?: boolean // чи це саме той випадок підміни (для напису)
}
